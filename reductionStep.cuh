...
__device__ void reductionStep(float *shared_qkt, float *maxValues,
  ...
  // row across warps
  printf("kElementsTracked: %d, qElementsTracked: %d\n", kElementsTracked,
         qElementsTracked);
  ...
  for (int i = warpid; i < req_tiles; i += WARPS_PER_BLOCK) {
    ...
    int output_u_left[2] = {
        ...
        (i) % req_x_tiles * TILE_X_SIZE}; // split output tile work across warps
    for (int j = 0; j < (b_c / SHARED_Q_K_DIM); j++) {
      ...
      half p_elements[8] = {casted_qkt[(p_u_left[0] + laneid / 4) * b_c +
                                       p_u_left[1] + 2 * (laneid % 4)],
                            casted_qkt[(p_u_left[0] + laneid / 4) * b_c +
                                       p_u_left[1] + 2 * (laneid % 4) + 1],
                            casted_qkt[(p_u_left[0] + laneid / 4 + 8) * b_c +
                                       p_u_left[1] + 2 * (laneid % 4)],
                            casted_qkt[(p_u_left[0] + laneid / 4 + 8) * b_c +
                                       p_u_left[1] + 2 * (laneid % 4) + 1],
                            casted_qkt[(p_u_left[0] + laneid / 4) * b_c +
                                       p_u_left[1] + 8 + 2 * (laneid % 4)],
                            casted_qkt[(p_u_left[0] + laneid / 4 + 8) * b_c +
                                       p_u_left[1] + 8 + 2 * (laneid % 4) + 1],
                            casted_qkt[(p_u_left[0] + laneid / 4) * b_c +
                                       p_u_left[1] + 8 + 2 * (laneid % 4)],
                            casted_qkt[(p_u_left[0] + laneid / 4 + 8) * b_c +
                                       p_u_left[1] + 8 + 2 * (laneid % 4) + 1]};
